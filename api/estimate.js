...
import { estimateProviderGpu, estimateStorage, KAKAO_GPU_HOURLY, NAVER_GPU_HOURLY, STORAGE } from "../lib/usage.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === "catalog") return res.json({ gpu_hourly: { kakao: KAKAO_GPU_HOURLY, naver: NAVER_GPU_HOURLY }, storage: STORAGE, currency: "KRW", vat_included: false });
    ...
    if (type === "gpu") return res.json(estimateProviderGpu(String(req.body?.provider || "kakao"), String(req.body?.flavor), req.body?.minutes, req.body?.volume_gb));
