...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (request.method === "POST" && action === "upload") { const data = Buffer.from(String(request.body?.base64 || ""), "base64"); if (data.length > 3_500_000) return response.status(413).json({ error: "file_too_large", max_bytes: 3500000 }); await uploadObject(bucket, key, data, String(request.body?.content_type || "application/octet-stream")); await startStorage("kakao", bucket, key, data.length); return response.status(201).json({ ok: true, bucket, key, size: data.length }); }
    if (request.method === "DELETE" && action === "object") { await deleteObject(bucket, key); await stopStorage("kakao", bucket, key); return response.json({ ok: true, deleted: key }); }
