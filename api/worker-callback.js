...
async function recordTerminalUsage(job, status) {
  ...
  const disk = (Number(job.volume_gb) || 0) * 0.16 * hours;
  const amount = gpu + disk;
  await addUsage({ provider: "kakao", category: "gpu", action: status, label: `${job.flavor_name || "GPU"} · ${job.key}`, amount, meta: { job_id: job.id, seconds, hourly_rate: job.hourly_rate, volume_gb: job.volume_gb, gpu, disk, status } });
  job = await updateJob(job.id, { usage_amount: amount, usage_gpu_amount: gpu, usage_disk_amount: disk, usage_seconds: seconds, usage_recorded_at: new Date().toISOString() });
  try {
  ...
  try {
    ...
    if (terminal(status) && job.instance_id) {
      try {
        ...
        job = await updateJob(id, { instance_deleted_at: new Date().toISOString() });
      } catch (error) {
