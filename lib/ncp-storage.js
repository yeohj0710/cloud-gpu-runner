import { parseBuckets, parseObjects, presignS3, s3Request } from "./s3-client.js";
...
export function presignObject(bucket, key, method = "GET", expires = 3600) { return presignS3({ ...config(), method, path: `${bucket}/${key}`, expires }); }
export async function enableBrowserUploads(bucket) { const xml = `<?xml version="1.0" encoding="UTF-8"?><CORSConfiguration><CORSRule><AllowedOrigin>https://work-memory-ten.vercel.app</AllowedOrigin><AllowedMethod>PUT</AllowedMethod><AllowedMethod>GET</AllowedMethod><AllowedHeader>*</AllowedHeader><ExposeHeader>ETag</ExposeHeader><MaxAgeSeconds>3600</MaxAgeSeconds></CORSRule></CORSConfiguration>`; await s3Request({ ...config(), method: "PUT", path: bucket, query: "cors=", body: Buffer.from(xml), contentType: "application/xml", contentMd5: true }); }
