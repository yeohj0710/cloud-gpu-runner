...
export async function bootstrapNcpGpu(regionCode = "KR") {
  ...
  let vpc = vpcs.find((item) => item.vpcNo === reusableSubnet?.vpcNo) || vpcs.find((item) => item.vpcName === "ccl-gpu-vpc");
  let createdVpc = false;
  if (!vpc) {
    const data = await ncp(ncpPath("/vpc/v2/createVpc", { regionCode, vpcName: "ccl-gpu-vpc", ipv4CidrBlock: "10.30.0.0/16" }));
  ...
  let privateKey;
  if (!keys.length) {
    const data = await ncp(ncpPath("/vserver/v2/createLoginKey", { regionCode, keyName: "ccl-gpu" }));
    privateKey = data.createLoginKeyResponse?.privateKey;
    keys = [{ loginKeyName: data.createLoginKeyResponse?.keyName || "ccl-gpu" }];
  }
  ...
  if (!existing) {
    ...
    await ncp(ncpPath("/vpc/v2/createSubnet", {
      regionCode, zoneCode: "KR-1", vpcNo: vpc.vpcNo, subnetName: "ccl-gpu-public",
