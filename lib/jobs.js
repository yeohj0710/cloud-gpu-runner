...
export function validateCustomJob(input) {
  ...
  const modelKey = String(input.model_key || "");
  ...
  if (![codeKey, dataKey, modelKey].filter(Boolean).every((key) => SAFE_OBJECT_KEY.test(key) && !key.includes(".."))) throw new Error("unsafe_object_key");
  ...
  const provider = ["auto", "naver", "kakao"].includes(String(input.provider || "auto")) ? String(input.provider || "auto") : "auto";
  const taskMode = input.task_mode === "inference" ? "inference" : "training";
  return { codeKey, dataKey, modelKey, command, outputPath, provider, taskMode };
}
export async function createJob(input) {
  ...
  if (input.type === "custom-gpu") {
    ...
    job = {
      id, type: "custom-gpu", status: "queued", provider: value.provider === "auto" ? undefined : value.provider,
      requested_provider: value.provider,
      bucket: input.bucket, key: value.codeKey, code_key: value.codeKey,
      data_key: value.dataKey || undefined, model_key: value.modelKey || undefined,
      model_id: String(input.model_id || "").slice(0, 100) || undefined,
      source_model_id: value.modelKey ? String(input.model_id || "").slice(0, 100) || undefined : undefined,
      preset_id: String(input.preset_id || "").slice(0, 80) || undefined,
      command: value.command, task_mode: value.taskMode,
      output_path: value.outputPath, result_key: `results/${Date.now()}-${id}.tar.gz`,
      preview_key: `results/previews/${id}.jpg`, manifest_key: `results/manifests/${id}.json`,
