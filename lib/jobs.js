...
export async function createJob(input) { const jobs = await listJobs(); const job = { id: randomUUID(), type: "whisper-transcription", status: "queued", provider: "ncp", bucket: input.bucket, key: input.key, language: input.language === "auto" ? "ko" : input.language || "ko", result_key: `results/${Date.now()}-${randomUUID()}.json`, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; await saveJobs([...jobs, job]); return job; }
