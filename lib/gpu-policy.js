...
export function assertGpuEligibleForJob(provider, flavor, job = {}) {
  const capability = gpuCapability(provider, flavor);
  if (provider === "kakao" && capability.model === "T4" && job.type === "custom-gpu" && job.preset_id === "qwen-lora-v1") {
    return { ...capability, profile: "qlora-4bit" };
  }
  return { ...assertHighValueCloudGpu(provider, flavor), profile: "bf16-lora" };
}
