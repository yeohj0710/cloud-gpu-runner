...
export async function searchPubMed(query, limit = 50) {
  ...
  const search = await fetch(
    `${BASE}/esearch.fcgi?db=pubmed&retmode=json&sort=pub+date&retmax=${size}&term=${encodeURIComponent(query)}&tool=cloud-credit-lab-console&email=research@wellnessbox.kr`,
    { cache: "no-store" },
  );
  ...
  const fetched = await fetch(
    `${BASE}/efetch.fcgi?db=pubmed&retmode=xml&id=${ids.join(",")}&tool=cloud-credit-lab-console&email=research@wellnessbox.kr`,
    { cache: "no-store" },
  );
